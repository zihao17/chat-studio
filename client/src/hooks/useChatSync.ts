...
export interface ChatSyncActions {
  ...
  saveMessage: (sessionId: string, role: 'user' | 'assistant', content: string, title?: string) => Promise<void>;
...
export function useChatSync(): ChatSyncState & ChatSyncActions {
   ...
   * 同步游客数据到云端
   */
  const syncGuestData = useCallback(async (guestSessions: ChatSession[]): Promise<boolean> => {
